...
public class GUIController{
	...
	/**
	 * This is the constructor for the Gui controller, it has all the action listeners for the buttons 
	...
	public GUIController (CommunicationController communicator) {
		...
		loginScreen.addLoginButtonListener((ActionEvent e) ->{
			...
			if (result==0) {
				...
				newGuy.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
				
				newGuy.addWindowListener(new java.awt.event.WindowAdapter() {
				    @Override
				...
				newGuy.addNewUserButtonListener((ActionEvent f) ->{
						newGuy.setVisible(false);
						MakeNewUser newUser = new MakeNewUser();
						newUser.display();
						
						newUser.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
						...
						newUser.addFinishButtonListener((ActionEvent g) ->{
							...
							if (outcome ==0) {
								...
								//Username too short
								JOptionPane.showMessageDialog(null, "Username was too short!", "Error", JOptionPane.ERROR_MESSAGE);
							}
		...
		/**
		 * this is the listeners for the search catalog button, it will deal whatever happens after the 
		...
		gui.addAddCourseListener((ActionEvent e) ->{
			...
			stu.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
			
			stu.addWindowListener(new java.awt.event.WindowAdapter() {
			    @Override
			    public void windowClosing(java.awt.event.WindowEvent windowEvent) {
					...
					if (result ==0) {
						...
						course.addContinueListener((ActionEvent d) ->{
							...
							if (response == 0) {
								...
								JOptionPane.showMessageDialog(null, "The section is already full!", "Error", JOptionPane.ERROR_MESSAGE);
							}
