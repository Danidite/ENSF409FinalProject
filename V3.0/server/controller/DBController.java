...
public class DBController implements Runnable {
	 ...
	 */
	private boolean askForCredencials() {
		while (true) {
			...
			String result = atemptLogin(userName, password);
			...
			if (result.equals("1")) {
				while (true) {
					String choice = serverCommunicator.reciveRawInput();
					if (choice.equals("1")) {
						//They wish to ADD a user
						userName = serverCommunicator.reciveRawInput();
						password = serverCommunicator.reciveRawInput();
						...
						//They do NOT Wish to add any MORE new user(s)
						return true;
					}
				}
			} else if (result.equals("0")){
				//They are NOT ADMIN, we leave this
				return true;
			}
		}
	}
		...
		while (true) {
			...
			case 6:
				...
				case 1:
					...
					serverCommunicator.communicateToClient(model.courseExist(data.get(2), data.get(3)));
					break;
				case 2:
					if (data.size()<3) {
						serverCommunicator.communicateToClient("1");
				...
				case 4:
					...
					break;
				case 5:
					serverCommunicator.communicateToClient(model.canEnroll(data.get(2), data.get(3), data.get(4),data.get(5)));
					break;
				}
				break;
			default:
